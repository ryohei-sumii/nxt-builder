...
- 実装系（#1,2,2b,7,8）の期待コードは `verification/app` に反映され `bash run.sh` でビルド可能性を担保する。
  2b は「APIキー管理」スライス（`app/settings/api-keys/*`＋`lib/api-keys/*`・`lib/data/api-keys.ts`・
  `lib/rate-limit.ts`）で、レート制限・秘密のハッシュ保存・一度だけ平文表示・IDOR スコープ失効を代表化。
  ただし **ビルドが担保するのは"出力コードの形が pinned スタックでコンパイル可能"までで、2b の核心である
  「要件を ID 分解して全 ID を網羅したか」という過程はビルドでは検証できない**（手動ルーブリックのまま）。
