...
- `middleware.ts` はルート単位の粗いガードには使えるが、**唯一の防御にしない**
  （Server Action/Route Handler 側でも再チェック）。
...
- Server Actions は Next.js が Origin 検証などの CSRF 保護を持つが、独自 Route Handler で状態変更する
  場合は Origin/CSRF トークンを自前で確認する。
- Cookie は `httpOnly` / `secure` / `sameSite` を適切に設定。トークンを `localStorage` に置かない。
- `next.config` の `headers()` で `Content-Security-Policy`・`X-Content-Type-Options: nosniff`・
  `Strict-Transport-Security`・`Referrer-Policy` 等のセキュリティヘッダを設定する。
