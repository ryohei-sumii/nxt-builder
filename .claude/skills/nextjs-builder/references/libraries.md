...
- **セキュリティ。** 秘密を扱うか（→ `server-only` 側に隔離）。メンテ状況・既知脆弱性・依存ツリーの
  大きさ（サプライチェーン面）を確認する。**追加の具体手順（名前の実在/正規性照合・バージョン固定・監査）は
  下記「依存を追加する手順（サプライチェーン）」**。
...
新規依存は攻撃面。標準機能で足りないと確認した後、**入れる前に実在・正規性・健全性を確かめ、入れた後に監査**する。

**入れる前:**
- **標準機能で代替できないか**を再確認（`Intl` / `crypto` / `fetch` / Server Actions 等）。要らない依存が最良。
- **名前の実在と正規性を照合する。** 公式リポジトリ/ドキュメントから正確なパッケージ名を確認し、npm ページの
  発行元・provenance・週間 DL・最終公開日・メンテ状況を見る。**綴り近縁の別物**（typosquat / slopsquat）に注意。
  **LLM が挙げた名は実在・正規と限らない**ので鵜呑みにしない。
- **ライセンス**がプロジェクト方針に合うか。**重量とツリー**（クライアントに乗るか・推移的依存の数）を見る。

**入れる:**
- **特定した PM 経由**で追加（上記対応表）。**バージョンを固定**し lockfile をコミットする。CI は
  `npm ci` / `--frozen-lockfile` 等で lockfile から再現インストールし、ドリフトを防ぐ。
- **postinstall はサプライチェーンの実行点。** 信頼できない依存では `--ignore-scripts`（pnpm は
  `onlyBuiltDependencies` の allowlist）を検討する。

**入れた後:**
- **監査を回す**（`npm audit` / `pnpm audit` / `yarn npm audit` / `bun audit`）。脆弱性が出たら更新/代替する。
- 継続的な更新検知（Dependabot / Renovate）と、PR での lockfile 差分レビューを前提にする。
- **重大・不可逆な選定（DB/ORM/認証/デプロイ）と新規依存の追加は着手前に確認**（このリポの方針）。
