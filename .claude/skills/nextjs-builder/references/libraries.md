...
**大原則: 既存プロジェクトの選択を尊重し、勝手に新規導入しない。** 追加が妥当な場合のみ下記で評価する。
...
### 文字列由来の入力は `z.coerce`
`formData` / クエリ / 環境変数は文字列。数値・真偽・日付は `z.coerce.number()` /
`z.coerce.boolean()` / `z.coerce.date()` で変換しつつ検証する。
...
| **認証** | Auth.js(next-auth) / Clerk など。認可判定はサーバー（Server Action/Route Handler/`lib/data`）で。 |
