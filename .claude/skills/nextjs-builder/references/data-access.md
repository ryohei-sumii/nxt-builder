...
DB スキーマは**マイグレーションを通してのみ変える**。本番データに関わる不可逆な操作を含むため、自己流で直に変えない。

- **既存のツールを尊重する。** Prisma Migrate / drizzle-kit / 生 SQL / Atlas 等のどれを使っているか
  （`prisma/migrations` ・ `drizzle/` ・ `migrations/` ・ CI 設定）を確認し、それに合わせる。新しい
  マイグレーション基盤を勝手に導入しない。
- **スキーマとマイグレーションは対でコミットする。** スキーマ定義（`schema.prisma` / Drizzle スキーマ）だけ
  変えて生成を忘れない。生成された SQL は**適用前にレビュー**する（ORM が意図せず drop/recreate していないか
  目視）。手で本番 DB を叩かない（差分が記録に残らず再現不能になる）。
- **前方専用・小さく刻む。** 本番マイグレーションは前方専用と考え、1マイグレーション1目的。適用は特定した
  PM 経由（`<pm> exec prisma migrate deploy` 等・`references/libraries.md`）。まずコピー/ステージングで試す。
- **破壊的変更は expand-contract（段階移行）で無停止に。** 列/テーブルの改名・削除・NOT NULL 化を
  「旧コードが動いている最中」に一発でやらない。**①追加（nullable で新列/新テーブル）→②二重書き込み＋
  バックフィル→③読み取りを新へ切替→④旧を削除**、と複数デプロイに分ける。列 rename は「新列追加→コピー→
  旧削除」に分解する。
- **破壊的/不可逆な操作（drop column・drop table・型変更・一意制約の後付け）は着手前に確認する**
  （このリポの「重大・不可逆は着手前に確認」と一致）。バックアップ/ロールバック手段の有無も確かめる。
- **データ移行はスキーマ移行と分ける。** バックフィル等のデータ変更は**冪等な**スクリプトにし、DDL と混ぜない
  （失敗時の切り分け・再実行のため）。開発用 seed を本番に流さない。
...
- [ ] スキーマ変更は既存ツールのマイグレーション経由で生成・レビューしたか？ 破壊的変更は expand-contract で
      無停止化し、drop/型変更等の不可逆操作は着手前に確認したか？
